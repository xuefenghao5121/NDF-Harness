...
> **人只说五句口令。Command 听口令、造 pack、等人审。Control 写提案和装订器。Implementation 只在允许的写根里改代码。成功只认磁盘上的 `ndf-agent-completion/v1`。探索进 `poc/`，主线契约和 Trunk 分开。**

原始 NDF 解决的是「规范写成什么样」。Harness 解决的是「人跟 Agent 每天按什么规矩干活」。装进消费仓后，说了算的是那边的 `spec/meta/` 和 `AGENTS.md`；这个包是种子。
...
复杂系统该听谁的？不该听实现本身，也不该听测试抽样。该听仓库里一份说清「应当 / 不得 / 可以」的规范。人跟 Agent 一起做设计时，这份东西经常根本没落成可查的文件。

**真正管事的是聊天。** 人丢半成品，Agent 吐设计和测试报告，人再补一句，几百轮。管设计的往往是「参考文档 + 初稿 + 整段 prompt」。散在聊天和脑子里，没法重放，后句盖前句，清聊天就丢。系统有了，规范没有。以后改代码只能考古。（原始提案 §1.3。）

**规范只活一个 PR。** Spec Kit 一类也用 Markdown、「先规范后代码」，但规范常跟单个变更请求一起生灭：合入就扔。复杂系统要的是能跟着项目活几年的那份，不是每发一个 PR 换一本。（原始提案 §3。）

**Agent 说做完了，你核不了。** ACK、聊天一句 OK、stdout 一段 JSON，就被当成委派成功，磁盘上没有可核的回执。或者面板和回放把一次日常 POC 拖成几小时，人盯仪式不盯假设。见 [`SECURITY.md`](SECURITY.md) 与 ADR-META-003 / ADR-META-004。

三个缺口：

1. 规范能不能持续改、带历史、人和机器都能读？
2. 没证伪的东西能不能默认写成 stable、直接改生产路径？
3. 谁批准、写到哪、怎样算做完，能不能机械判定？

第一件靠怎么写规范。后两件必须有工作流。Harness 的内核就是冲着后两件去的，同时把第一件接进日常口令。

## 2. 原始 NDF 给了什么（Harness 的底座）

**NDF（Normative Description Format）** 是 Markdown + Git 上的约定。不发明新格式，不搞服务器。纯文本加 `git` 就能写。

纯散文不行：含混、不可合并、不可查询。全盘形式化（ASL、TLA+、证明助手）也不行：写起来可能比设计本身还重，早期含混和工程判断塞不进去。NDF 走中间：自然语言为主；结构加在组织层（标识、分类、连接、版本）；代码更合适的地方就嵌规范性代码片段。

它自己的纪律句是：

> 散文活在树里，语义活在图里，时间活在 git 里。稳定的条款 ID 把三者串起来。

树（`spec/00-charter/` 到 `50-verification/`）管归属。图（`refines`、`depends-on`、`conflicts-with`、`verifies`...）管语义。Git 加决策记录管历史；改意图就留下记录，别悄悄删。

条款是标题块 + `{#稳定ID}` + `<!-- ndf: ... -->`，正文用 MUST / SHOULD / MAY。L0 意图、L1 契约、L2 机制、L3 可执行模型都留着，用 `refines` 串。

协作上，提案 §7 设想：人写章程和 L0/L1；Agent 接「基线 `spec-vX` + 条款 ID」的工单；产出回流成条款和决策记录；人批规范变更，Agent 提议。§9 也承认：一致性靠评审，不是证明；蒸馏会丢信息；纪律会松。

NDF 把「规范怎么写成可持续的仓库工件」说清楚了。它没说清日常脏活：试错写哪儿、人说哪句才算授权、怎样算 Agent 做完、要不要让人选工具。这些是 Harness 的活。
...
```text
原始 NDF：条款怎么写、图怎么连、prompt 怎么收成决策记录
Harness 内核：谁说哪句才派、写到哪、怎样算完成、探索怎么不脏主线
```

### 内核四件套

1. **五句口令**（人对指挥面）：初始化项目 / 提交Idea / 派发 / 继续 / 关闭。不选 skill，不开面板，不背 CLI。入口见 [`skill/ndf-workflow/`](../skill/ndf-workflow/SKILL.md)。
2. **三角色写根**：Command 听口令、造 pack；Control 写提案和装订器；Implementation 只动 `allowed_write_root`。没绑角色就 `roles_unbound`，派不出去。
3. **双轨**：探索只在 `poc/<topic>/`；探索期 MUST NOT 改 Trunk `src/` / `include/` / `tests/`（[[CHR-008]] / [[BEH-018]]）。
4. **成功合同**：磁盘 `ndf-agent-completion/v1`。ACK / stdout 不算。派发要绑 `GATES` 里的 bundle SHA（[[META-010]]）；文件在 ≠ 人批过。
...
其它常见空洞也对上了：同一 `manifest_sha` 给 Control 和 Implementation；Genesis 是绑内核、一句「派发」、再 `GENESIS已审核`（[[META-009]]）；证伪走 reject + DEC，不许静默删条款留代码（[[BEH-020]]）。

## 4. 内核带来的好处

**人这边极简。** 出 Idea、审契约、说口令、看 blockers 和磁盘回执。指挥面听口令、造 pack、等人审、调工具、把失败原因甩回来。
...
**探索脏不了主线。** `draft` / `stable` 是状态；`poc/` 是物理隔离。没有这层，条款树会被试错写乱。

**文字指挥，该停就停。** 日常不走 Commander / Episode / Replay。错仓库、越界写根、缺人审 bundle、并发写、上下文漂移、伪造 completion、超预算：过不了就停。见 [`SECURITY.md`](SECURITY.md)。
...
人这边尽量只说：角色，派发，GENESIS已审核。别在 GENESIS 后再走一轮日常 promote 才升骨架。做完这步，后面优化至少有对照目标。
...
聊天里长不出能查的规范。NDF 管规范怎么写成树、图、git。Harness 的内核是五句口令、三角色写根、`poc/` 双轨、磁盘 completion：人拍板，Agent 在界内干活，做完以盘上的回执为准。
